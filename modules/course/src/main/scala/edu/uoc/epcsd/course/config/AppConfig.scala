package edu.uoc.epcsd.course.config
...
final case class MicrocredentialServiceConfig(baseUrl: String)
...
final case class AppConfig(
    ...
    microcredentialService: MicrocredentialServiceConfig,
    retry: RetryConfig,
    rabbit: RabbitConfig
)
...
object AppConfig:
  ...
  def load(config: Config): Either[String, AppConfig] =
    try
      ...
      val userSvc = config.getConfig("userService")
      val mcSvc = config.getConfig("microcredentialService")
      ...
      Right(
        AppConfig(
          ...
          MicrocredentialServiceConfig(mcSvc.getString("baseUrl")),
          RetryConfig(retry.getInt("maxAttempts"), retry.getLong("baseDelayMillis")),
          RabbitConfig(
            rabbit.getBoolean("enabled"),
            rabbit.getString("host"),
            rabbit.getInt("port"),
            rabbit.getString("virtualHost"),
            rabbit.getString("username"),
            rabbit.getString("password"),
            rabbit.getString("exchange"),
            rabbit.getString("queue"),
            rabbit.getString("routingKey")
          )
        )
      )
